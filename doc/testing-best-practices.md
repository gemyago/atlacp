...
If you need to reuse the same structure in multiple tests, create a factory function with options pattern to reuse it.
...
expectedPR := bitbucket.NewRandomPullRequest(
    bitbucket.WithPullRequestID(int(faker.RandomUnixTime())),      // Already generated
...
Benefits:
- **DRY principle** - Don't repeat default values that are already provided
- **Single source of truth** - Default values are defined in one place
- **Less maintenance** - Changes to default patterns only need to be made in the utility function
- **Cleaner tests** - Tests focus on what's actually being tested, not setup
- **More readable** - Intent is clearer when you only specify what you need to override
