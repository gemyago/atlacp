...
- **Orchestrator responsibilities**: confirm the CLI is runnable for sub-agents (see [Prerequisites](#prerequisites-done-by-the-user)); launch one sub-agent per test to run; collect outputs and produce the final report.
...
3. **For each test:** spawn a dedicated sub-agent with the Bitbucket repo path, `repo_owner` / `repo_name`, and test number.
