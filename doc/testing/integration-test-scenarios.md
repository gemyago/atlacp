...
- **Orchestrator-only work is allowed** (and encouraged where useful): planning order, spawning sub-agents, passing repo path / `repo_owner` / `repo_name` / test id, **verification that does not execute the test** (e.g. confirming sub-agents have bitbucket intetration, merging `tmp/integration-tests-*.md` from outputs, summarizing pass/fail). The orchestrator does **not** substitute for a sub-agent when executing a test.
- **Why sub-agents**: isolates failures, avoids mixing shell/Integration call steps for different tests in one thread, and matches review expectations. This is **not** permission to run multiple tests in parallel on the same repo.
- **Orchestrator responsibilities**: confirm Bitbucket integration is available to sub-agents; when running multiple tests, launch sub-agents **sequentially** (wait for each to complete before the next); collect outputs and produce the final report.
- **Sub-agent responsibilities**: perform **all** shell work in the provided repo path and **all** bitbucket integration calls for **that test only**; write/update the workspace results file for that test’s steps; return a concise pass/fail summary and PR ids to the orchestrator.
...
**Important**: You need to use the shell commands to work with the repository and and all files. Do not use the file system tools to work with the repository and files. You will not be able to since they're in a different folder (other than current workspace folder).
...
```markdown
# Bitbucket Integration Test Results
...
2. Create a new branch from main `feature/bitbucket-atlacp-integration-tests-results-{timestamp}`
...
5. Use Bitbucket Integration actions to create, approve, and merge the PR:
    - title: "Bitbucket Atlacp Integration Tests Results {timestamp}"
    - description: "This is an automated integration tests results (Test 1, Test 2, Test 3, Test 4, Test 5, Test 6) {timestamp}"
    - source_branch: "feature/bitbucket-atlacp-integration-tests-results-{timestamp}"
    - target_branch: "main"
    - repo_owner: your workspace name
    - repo_name: your repository name
    - merge_strategy: "squash"
...
1. Confirm the ATLACP Bitbucket integration is available **to sub-agents** (sub-agents need the same integration access as the parent; if a sub-agent cannot call integration, **do not** run the test in the orchestrator—report that limitation to the user instead).
2. **Strict rule:** **Every** test’s executable steps run **only** in a dedicated sub-agent. The orchestrator may verify prerequisites, merge reports, and summarize; it **must not** run git or any bitbucket integration calls for a test’s procedure in the main thread.
